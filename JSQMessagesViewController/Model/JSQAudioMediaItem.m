...
@property (strong, nonatomic) UIView *cachedMediaPreview;
...
{
    if (duration < 1) {
        return [NSString stringWithFormat:@"00:01"];
    }
...
- (UIView *)mediaPreview
{
    if (self.audioData && self.cachedMediaPreview == nil) {
        if (self.audioData) {
            self.audioPlayer = [[AVAudioPlayer alloc] initWithData:self.audioData error:nil];
            self.audioPlayer.delegate = self;
        }
...
//        NSLog(@"%f", self.progressLabel.frame.size.width);
        ...
        self.progressView = [[UIProgressView alloc] initWithProgressViewStyle:UIProgressViewStyleDefault];
        CGFloat width = 60;
        CGFloat xOffset = (previewBoxSize / 2) - (width / 2);
        self.progressView.frame = CGRectMake(xOffset, 63,
                                             width, self.progressView.frame.size.height);
        self.progressView.tintColor = self.audioViewAttributes.tintColor;
        [playView addSubview:self.progressView];
        
        self.cachedMediaPreview = playView;
    }
    
    return self.cachedMediaPreview;
}
